...
def _run(experiment: str, out: Path, bank: Path, extra: list[str] = ()) -> Path:
    cfg = compose_config([
        ...
        f"driver.bank_dir={bank}", f"num_gpus={NUM_GPUS}", "tracking.mode=disabled", *extra,
    ])  # tracking disabled: a CPU test must never open a real W&B run
    return run(cfg)
...
def test_pool_link_guard() -> None:
    ...
    _write_bank(bank)
    out = _tmp("guard")
    cfg = compose_config(["+experiment=r1_correctness_8k", "driver.dry_run=true", f"output_dir={out}", f"driver.bank_dir={bank}", "tracking.mode=disabled"])
