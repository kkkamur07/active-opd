...
- 2026-09-05: **Chain complete (CHAIN DONE 20:16 UTC)**; r3 resumed at random_questions step 10 after the disk cleanup (USER ran the rm via the peer session at ~15:45 UTC) and the finished-arm prune left only round_09 weights per arm (114 GB free). Results, MATH-500 strict avg@4, base 0.264-0.269, noise ~+-2 pts: r1 TRSW 0.455 / both_right 0.443 / mixed 0.433 / both_wrong 0.410; r2 kl_mid 0.438 / kl_high 0.435 / kl_low 0.433 / random4 0.431 / entropy_top4 0.424; r3 random_questions 0.441 / uncertain_questions 0.422 (uncertain starts slower: 0.372 at step 10, own-rollout strict accuracy 0.01-0.10). All 11 arms: essentially the whole gain by step 10 (cap-hit 0.79 -> ~0.6), flat or slightly down to step 100; only both_wrong and uncertain_questions sit below the pack beyond noise, no rule beats random. Interpretation deferred to the USER; the open bf16-rounding question above is the leading candidate for why every curve saturates at step 10.
- 2026-09-06: **r4-qentropy-low-8k launched: the reverse of r3's question rule** (USER: "if entropy selection is hurting try reverse entropy selection please maybe?"). r3's uncertain_questions (top-800 H(q)) finished below random_questions (0.422 vs 0.441 at step 100; pooled steps 50-100 0.424 vs 0.437, paired t = -4.15 per the peer session). New question source `bank_low_entropy` (apod/driver.py; the 800 LOWEST student H(q), mean 0.32 vs top-800's 0.82 and random's 0.52), one arm `certain_questions`, conf/experiment/r4_qentropy_low_8k.yaml; control = r3's random_questions arm, not re-run (same bank, regime, base student, eval problems; r1 already borrows it). Bank composition note from the peer: the top-800 are 68% both_wrong among labelled rows and the bottom-800 32% both_right, so r1's bucket ordering predicts the reverse arm lands at or above random; the top/bottom-800 are ~60-70% unlabelled rows, so that mechanism is suggestive only. Also noted: `bank_random` = the first 800 bank rows, which are 100% teacher-labelled because labelling ran in file order; H(q) is flat across file order, so the control stays a fair draw on the entropy axis.
- 2026-09-07: **r4 result and USER DECISION: experiments stopped** ("stop the experiments" via the peer session; nothing further launched -- no second random control, no mid-entropy arm). r4 certain_questions (bottom-800 H(q)) MATH-500 strict avg@4: 0.461 at step 10, 0.424 at step 100. Pooled steps 50-100 (peer session, eval set md5-identical across r1-r4): random_questions 0.4374; uncertain_questions 0.4237 (-0.014, paired t -4.15); certain_questions 0.4212 (-0.016, t -4.62); certain vs uncertain -0.003 (t -0.69, n.s.). Both ends of the H(q) axis lose to random by the same ~1.5 points, which rules out the bucket-mix mechanism noted on 2026-09-06 (the bottom-800 is both_right-heavy and did not gain). Surviving reading: narrowing the question distribution costs ~1.5 points whichever end it is narrowed to -- consistent with r2's null (trajectory rules do not narrow the question pool) and with r1's both_wrong (narrow and badly matched). UNRESOLVED caveat: one random control, a 1.5-point effect, a measured 6.9-point seed spread on round-to-round deltas; the two selected arms agreeing makes a lucky control less likely but does not exclude it. A second control run was offered and declined.
- 2026-09-07: **AIME 2025+2026 columns carry no signal at cap 8192 -- do not read them as results.** Every arm of r1-r4 sits at ~0.002 strict avg@16; 99.9% of AIME responses hit the cap (mean length 8191 vs MATH-500's 63% truncated at mean 6826). The monitor set would need cap 16k+ to say anything.
